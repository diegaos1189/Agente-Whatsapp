...
export async function ConversationsView({
  restaurantId,
  basePath = "",
  searchParams,
}: {
  ...
  const conversations = await apiServerFetchScoped<ConversationSummaryDTO[]>("/api/conversations", restaurantId);
      ...
      <div className="wa-chat-list" style={{ marginTop: 16 }}>
        {filtered.map((c) => (
          <div key={c.id} className="wa-chat-row">
            <Link href={`${basePath}/conversations/${c.id}`} className="wa-chat-link">
